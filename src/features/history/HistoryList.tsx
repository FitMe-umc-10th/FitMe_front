...
import { getHistoryList, updateHistoryStatus, HistoryStatus, HistoryItem } from '@/apis/history';

type TabType = 'WAITING' | 'FINAL_PASS';
...
export default function HistoryList() {
  ...
  // 이력 데이터 조회
  const { data: histories = [], isLoading } = useQuery<HistoryItem[]>({
  ...
  const updateStatusMutation = useMutation({
    mutationFn: ({ id, status }: { id: number; status: HistoryStatus }) =>
  ...
  const filteredHistories = histories.filter((item) => {
    if (activeTab === 'FINAL_PASS') {
      return item.status === 'FINAL_PASS';
    }
    // WAITING 탭에는 최종합격이 아닌 모든 건(DRAFT, WAITING, DOCUMENT_PASS)이 노출됨
    return item.status !== 'FINAL_PASS';
  });

  const tabs = [
    { label: '결과 대기', value: 'WAITING' as const },
    { label: '최종 합격', value: 'FINAL_PASS' as const },
  ];
  ...
  return (
    <Layout header={<Header title="이력" />} tabBar={<TabBar />} className="bg-slate-50/50">
      ...
      {isLoading ? (
        ...
        <div className="grid grid-cols-2 gap-x-[14px] gap-y-[16px] px-[15px] py-[20px]" ref={dropdownRef}>
          {filteredHistories.map((item) => (
            ...
            >
              ...
              <div className="w-[173px] h-[102px] rounded-t-[16px] overflow-hidden relative bg-slate-50 flex-shrink-0 z-0">
                {item.posting?.posterUrl ? (
                  <img
                    src={item.posting.posterUrl}
                    alt={item.posting.title}
              ...
              <div className="w-[173px] h-[152.13px] rounded-[16px] border-r border-b border-l border-slate-100 bg-white pt-[12px] pb-[12px] px-[12.43px] flex flex-col justify-between absolute bottom-0 left-0 z-10">
                ...
                <div className="w-[148.14px] h-[38.55px] flex flex-col gap-[2px] min-w-0">
                  <h3 className="font-['Pretendard'] font-bold text-slate-800 text-[12px] leading-[1.3] tracking-[-0.24px] truncate group-hover:text-blue-600 transition-colors">
                ...
                <div className="w-[146.07px] h-[42.36px] bg-[#f0f5ff]/70 border border-blue-50/20 rounded-[8px] pt-[5.18px] pr-[7.25px] pb-[5.18px] pl-[7.25px] flex items-center justify-start mx-auto overflow-hidden">
                  <p className="font-['Pretendard'] text-[10px] text-slate-500 font-semibold leading-[1.3] line-clamp-2 text-left">
                ...
                <div className="relative mt-[12.43px] flex justify-center">
                  ...
                  {openDropdownId === item.id && (
                    <div className="absolute bottom-9 left-1/2 -translate-x-1/2 z-40 w-[100px] bg-white border border-slate-100 rounded-xl shadow-[0_4px_16px_rgba(0,0,0,0.12)] py-1 font-['Pretendard'] overflow-hidden">
                      {(['DRAFT', 'WAITING', 'DOCUMENT_PASS', 'FINAL_PASS'] as HistoryStatus[]).map(
                        (statusOption) => (
                          <button
                            key={statusOption}
                            type="button"
